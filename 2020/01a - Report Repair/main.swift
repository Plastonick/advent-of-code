...
if let numbers = numbersThatSumTo(target: target, fromList: expenses) {
    let product = numbers.0 * numbers.1
    print("The two numbers that sum to \(target) are \(numbers.0) and \(numbers.1) and their product is \(product)")
}

for expense in expenses {
    let secondaryTarget = target - expense
    
    if let numbers = numbersThatSumTo(target: secondaryTarget, fromList: expenses) {
        let product = expense * numbers.0 * numbers.1
        print("The three numbers that sum to \(target) are \(expense), \(numbers.0), and \(numbers.1) and their product is \(product)")
    }
}
