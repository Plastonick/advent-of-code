...
class State:
    ...
    def key(self):
        sorted_state = [a for a in self.state.keys()]
        sorted_state.sort(key=lambda x: (100 * x[0]) + x[1])

        key = ""
        for pos in sorted_state:
            key += str(pos[0]) + "." + str(pos[1]) + "-" + self.state[pos]
    ...
    def move(self, a, b):
        ...
        return State(new_state, self.cost + move_cost, new_moves), move_cost
...
def calculate_min_cost(state: State):
    ...
    for move_from, move_to in potential_moves:
        new_state, move_cost = state.move(move_from, move_to)

        min_cost = calculate_min_cost(new_state)
        if min_cost is not None:
            costs.append(min_cost + move_cost)
...
best_cost = calculate_min_cost(state=starting_state)
